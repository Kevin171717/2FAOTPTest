...
class OTPCodeExtractor:
    ...
    @staticmethod
    def normalize_otp_candidate(raw_value: str, pattern_name: str | None = None) -> str:
        compact = re.sub(r"\s+", "", raw_value)
        if pattern_name == "grouped_numeric":
            return compact.replace("-", "")
        return compact

    def find_otp_candidate_records(self, text: str) -> list[dict]:
        return [
            {
                "raw": match.group(0),
                "code": self.normalize_otp_candidate(match.group(0), match.lastgroup),
                "start": match.start(),
                "end": match.end(),
                "pattern": match.lastgroup,
            }
            for match in self.otp_pattern.finditer(text or "")
        ]
    ...
    def extract_otp_code(self, message_data, source, credential_preference) -> tuple[list[dict], str | None]:
        ...
        def extract_otp_candidates(text: str, segments: list[tuple[str, int, int]]) -> list[dict]:
            ...
            for match in otp_matches:
                ...
                candidate = {
                    ...
                    "candidate_pattern": match.lastgroup,
                }
        ...
        if source == "email" or source == "sms":
            ...
            if not subject:
                ...
                subject = getattr(message, "subject", "") if message is not None else ""
            if subject:
                text = f"{subject}\n{text}"
